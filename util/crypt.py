...
def make_hashed_password(plain_text_password):
    return bcrypt.hashpw(plain_text_password.encode("utf-8"), bcrypt.gensalt(8)).decode(
        "utf-8"
    )
...
def validate_token(validate_only=False):
    ...
    try:
        decoded_token = jwt.decode(
            token_str.encode("utf-8"), _secret, algorithms=["HS256"]
        )
    except jwt.exceptions.InvalidSignatureError:
        return abort_or_validate(401, "token_failed_signaturecheck")

    valid_to_dttm = decoded_token.get("valid_to_dttm", None)
    if valid_to_dttm is None:
        return abort_or_validate(401, "token_invalid_fields")
    valid_to_dttm = dateutil.parser.parse(valid_to_dttm)
    if valid_to_dttm < datetime.datetime.now():
        return abort_or_validate(401, "token_expired")

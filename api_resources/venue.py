...
class Venue(Resource):
    def get(self, venue_id=None, user_id=None):
        ...
        if user_id and not venue_id:
            venue = (
                model.Venue.query.join(model.VenueUser)
                .join(model.User)
    ...
    def post(self):
        ...
        this_venue = model.Venue(name=name, address=address, timezone=timezone)
        db.session.add(this_venue)
        db.session.flush()
        db.session.refresh(this_venue)

        this_venue_user = model.VenueUser(
            venue_id=this_venue.id, user_id=user_id, is_admin=True
        )

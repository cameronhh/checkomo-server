...
class VenueCode(Resource):
    def post(self, venue_id):
        ...
        if model.Venue.query.get(venue_id) is None:
            abort(404)
        if not model.User.authedto_venue(user_id, venue_id):
            abort(403)

        venue_code_template_id = (
            None  # this should only be set if created from a template, NI yet
        )
        code = data.get("code")
        name = data.get("name")
        ...
        start_dttm = (
            dateutil.parser.parse(start_dttm) if start_dttm is not None else None
        )
    ...
    def put(self, venue_code_id=None, **kwargs):
        ...
        if not model.User.authedto_venue(user_id, this_venue_code.venue_id):
            return 401

...
class Visit(Resource):
    def get(self, visit_id=None, venue_id=None, venue_code_id=None):
        ...
        # protected endpoints
            ...
            if not model.User.authedto_venue(user_id, venue_id):
                abort(401)
            # get search params
            ...
            q_by_venue_code = (
                model.Visit.query.join(model.VenueCode)
                .join(model.Venue)
                .filter(model.Venue.id == venue_id)
            )
            q_by_venue_id = model.Visit.query.join(model.Venue).filter(
                model.Venue.id == venue_id
            )
    ...
    def post(self, venue_id=None, venue_code=None, venue_code_id=None):
        ...
        venue_code_id = (
            venue_code_id
            or data.get("venue_code_id")
            or model.VenueCode.query.filter_by(
                code=venue_code, venue_id=venue_id
            ).first()
        )  # not restful :(

        in_dttm = dateutil.parser.parse(
            data["in_dttm"]
        )  # datetime.strptime(data["in_dttm"],'%Y-%m-%d %H:%M:%S.%f')
        out_dttm = (
            None
            if data.get("out_dttm") is None
            else dateutil.parser.parse(data["out_dttm"])
        )
        ...
        if (
            this_venue_code.start_dttm is not None
            and datetime.now() < this_venue_code.start_dttm
        ):
            abort(400, "venue_code_too_new")
        elif (
            this_venue_code.end_dttm is not None
            and datetime.now() > this_venue_code.end_dttm
        ):
